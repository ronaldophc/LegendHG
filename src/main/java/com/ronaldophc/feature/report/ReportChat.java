...
public class ReportChat implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(command.getName().equalsIgnoreCase("reportchat")) {
            ...
            if(strings.length == 0) {
                player.sendMessage(Util.usage("/reportchat <player>"));
                return true;
            }
            if(strings.length == 1) {
                try {
                    ...
                    Report.reports.put(target, ReportEnum.CHAT);
                    player.sendMessage(Util.color1 + "Você reportou o jogador " + target.getName() + " por " + Util.color3 + "Chat");

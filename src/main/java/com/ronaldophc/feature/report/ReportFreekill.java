...
public class ReportFreekill implements CommandExecutor {

    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(command.getName().equalsIgnoreCase("reportfreekill")) {
            ...
            if(strings.length == 0) {
                player.sendMessage(Util.usage("/reportfreekill <player>"));
                return true;
            }
            if(strings.length == 1) {
                try {
                    ...
                    Report.reports.put(target, ReportEnum.FREEKILL);
                    player.sendMessage(Util.color1 + "Você reportou o jogador " + target.getName() + " por " + Util.color3 + "FreeKill");

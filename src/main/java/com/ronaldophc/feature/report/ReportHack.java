...
public class ReportHack implements CommandExecutor {

    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(command.getName().equalsIgnoreCase("reporthack")) {
            ...
            if(strings.length == 0) {
                player.sendMessage(Util.usage("/reporthack <player>"));
                return true;
            }
            if(strings.length == 1) {
                try {
                    ...
                    Report.reports.put(target, ReportEnum.HACK);
                    player.sendMessage(Util.color1 + "Você reportou o jogador " + target.getName() + " por " + Util.color3 + "Hack");
